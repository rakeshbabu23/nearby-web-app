...
const createPost = async (userId, postInfo, file) => {
  ...
  if (file) {
    try {
      mediaFiles = await uploadFilesToS3(file, userId, "posts");
    } catch (error) {
  ...
  const post = new Post({
    ...
    text: postInfo.text,
    tags: postInfo.tags ? JSON.parse(postInfo.tags) : [],
  });
  try {
    await post.save();
    return post;
  ...
  const pipeline = [
    {
      $geoNear: {
        ...
        maxDistance: Number(filters?.maxDistance) || 10000,
        spherical: true,
        query,
      },
    },
