...
io.on("connection", (socket) => {
  console.log("socket connection mae");
  socket.emit("jiii", "connection");
  socket.on("demo", (data) => {
    console.log("demoaa", data);
  });
  ...
  socket.on("register_user", ({ userId }) => {
    console.log("Registerin sssssssssssssssssssssssssss");
  ...
  socket.on("user_online_status", ({ roomName, userId, receiverId }) => {
    console.log("user_online", { roomName, userId, receiverId });
    const socketId = userSocketMap[receiverId];
    const room = io.sockets.adapter.rooms.get(roomName);
    //console.log("socket", room.has(socketId));
